...
class MCPAcademicServer:
    ...
    def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 3. Đóng gói phản hồi theo MCP JSON-RPC 2.0
        response = {
            "jsonrpc": "2.0",
...
if __name__ == "__main__":
    ...
    sched_tool = next(
        (t for t in tools if t.get("name") == "schedule_appointment"), None
    )
    if sched_tool and not sched_tool.get("parameters", {}).get("properties"):
        print(
            "⏳ [TODO 1.2]: Tool 'schedule_appointment' chưa được định nghĩa properties trong 'src/tools.py'."
        )
    else:
